...
This program requires Python 2.7.X.
    ...
    try:
        if 'comp_alias' not in crashes[pid]:
            ...
            if comp_alias is None:
                msg = "\tCouldn't find the pid %s in the enterprise log file."
                print msg % str(pid)
            ...
            if comp_alias is None:
                msg = "\tAll attempts to locate the component alias for %s \
failed."
                print msg % str(pid)
            else:
...
Enterprise log file %s... ' % (str(pid), enterprise_log),
    ...
    if (os.path.exists(enterprise_log)):
        """
        ServerLog       ProcessExit     1       0000257f551658c2:0
        2015-04-09 18:07:27     eCommunicationsObjMgr_esn       23340
        SBL-OSD-02006   Process 23340 exited with error - El proceso se
        """
        regex = re.compile(
                '.*(Process\s' + str(pid) + '\sexited\swith\serror).*',
...
failure: "line".'
        ...
        msg = 'File %s does not exists, cannot search for pid.'
        print msg % enterprise_log
    return None
                ...
                if (not(len(header_fields) >= 19)):
                    ...
                    if ((header_fields[13] == pid) and (
                        ...
                        shutil.copy2(original, (
                                os.path.join(crash_dir, log_filename)
                                ))
...
def find_last(log_archive_dir):
    """
    Search the newest directory created under the log archive of a Siebel
    Server.
...
def readConfig():
    ...
    directory with the filename equal to ".crash.ini".
...
if __name__ == '__main__':
    if ('SIEBEL_HOME' not in os.environ) and ('SIEBEL_LIB_DIR' in os.environ):
        msg = 'Environment variables SIEBEL_HOME and/or SIEBEL_LIB_DIR not \
available, check environment configuration before running this program!'
        raise NameError(msg)
    ...
    for filename in os.listdir(bin_dir):
        ...
        if filename[0:5] == 'core.':
            ...
            output = (
                    Popen(['file', core_path], stdout=PIPE).communicate()[0]
                    ).rstrip()
            """
            /<PATH>/siebsrvr/bin/core.23340: ELF 32-bit LSB core file Intel
            80386, version 1 (SYSV), SVR4-style, from 'siebmtshmw'
            """
            ...
            popen = Popen(['/usr/bin/gdb', bin,
                           ...
                           '-batch',
                           '-x',
                           gdb_cmd_filename], bufsize=0, shell=False,
                          stdout=PIPE)
            ...
            manage_comp_alias(crashes=crashes, pid=pid,
                              default_log=enterprise_log,
            ...
            if thread_id is not None:
                ...
                msg = "\tCouldn't find the thread id of the crashing thread of\
 process %s by reading the export FDR information" % str(pid)
                print msg
        ...
        for pid in crashes.keys():
            ...
            if 'comp_alias' in crashes[pid]:
                ...
                if 'thread' in crashes[pid]:
                    msg = 'Searching for log files of component %s, pid %s, \
thread id %s...'
                    print msg % (crashes[pid]['comp_alias'], str(pid),
                                 crashes[pid]['thread']),
                    ent_regex = re.compile(
                            crashes[pid]['comp_alias'] + '.*\.log')
                    ...
                    if log_counter > 0:
                        ...
                        thread_num = crashes[pid]['thread']
                        log_counter = find_logs(log_dir=last_one,
                                                regex=ent_regex,
                                                crash_dir=crash_dir,
                                                pid=str(pid),
...
logs.' % str(pid)
        ...
        print '\n----\nDumping technical details of component crashes \
found:\n %s\n----' % str(crashes)
        print 'Also dumping all technical details found in JSON format to \
crashes_info.json file'
    ...
    print 'End of report. If files were found, they will be located in the \
home directory of the server, under %s directory.' % crash_dir
