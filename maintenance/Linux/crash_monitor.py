...
If any is found, it will execute all the require steps to retrieve information from the core dumps, FDR, crash file and related log files
to generate a complete information about the crash. A summary with the information is generated as a JSON to STDOUT.
...
def dec2bin(decimal):
	...
	if decimal <= 1:
		...
		return dec2bin(decimal >> 1) + str(decimal & 1)
...
def manage_comp_alias(crashes, pid, default_log, archive_dir, crash_dir):
	"""
	Handles a component alias in the sense of locating and copying it's related crash information whenever is possible.
	...
	It will update in place parameter 1 and copy any component log file found to 5.
	"""
	try:
...
def find_comp_alias(pid, enterprise_log):
	"""
	Tries to find a process crash in the Siebel Enterprise Log file.
	Expects as parameter the PID of the process and the complete path to the Siebel Enterprise log file.
	"""
...
def find_logs(log_dir, regex, crash_dir, pid, thread_num):
	...
	- a regular expression object used to match the file names found in the log directory
	- the directory where the crash information will be stored, since the log files found will be copied to there
	- the related PID of the process that crashed
	- the thread number of the process that crashed

	Returns an integer with the number of logs found. Each log found will be copied to crash_dir.
	"""
	logs_counter = 0
...
def signal_map():
	...
	It's an ugly hack, but couldn't find a portable code to produce the correct result.
...
def signal_handler(signal, frame):
	"""
	SIGINT handler. Just print to STDOUT a message asking to wait operations to finish.
	This is an attempt to try to finish the process correctly in case of server bounce.
	"""
...
def find_last(log_archive_dir):
	"""
	Search the newest directory created under the log archive of a Siebel Server.
	Expects as parameters the complete path to the log archive directory.
	Returns a string of the complete path to the newest directory.
	"""
...
def readConfig():
	...
	This script uses a INI file to providing information of where to search for crash files and execute the expected
	steps once one if found.
	The configuration file is expected to the located at the user's home directory with the filename equal to ".crash.ini".
	Details on INI format can be checked by reading the standard iniparse module documentation.

	The expected keys and values are described below. You don't need to use namespaces in the INI file, neither it is expected to.
	...
	- crash_archive: expects a full path of a directory that will be used to store the crash(es) information. Inside this directory, subdirectories will
	  be created with the name corresponding to the string returned by today() method from datetime.date Python standard module and respective files will
          be located there.
	...
	- log_archive: the complete path to the directory where the log files were rotated
        - clean_files: defines if the related files (core, FDR and crash.txt) should be removed or not. Accept the values "yes" and "no". Log files will not 
          be removed at all.
	"""
