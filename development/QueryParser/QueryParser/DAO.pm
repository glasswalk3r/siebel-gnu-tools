...
sub new {
    ...
    die('user is an obligatory parameter in the configuration file')
      unless ( exists( $self->{user} ) and defined( $self->{user} ) );

    die('password is an obligatory parameter in the configuration file')
      unless ( exists( $self->{password} ) and defined( $self->{password} ) );

    die('driver is an obligatory parameter in the configuration file')
      unless ( exists( $self->{driver} ) and defined( $self->{driver} ) );
    die( 'driver string is incorrect: ' . $self->{driver} )
      unless ( $self->{driver} =~ /^dbi\:\w+$/ );

    die('database is an obligatory parameter in the configuration file')
...
sub parse_query {
    ...
    die 'A query is an obligatory parameter'
    ...
    my $sth = $self->get_dbh()->prepare($query)
      or die "Cannot parse the query: $DBI::errstr";
